...
ALPHA = 0.001        # Time-weight α (seconds coefficient) - higher for time sensitivity
BETA = 0.1           # LOC-weight β (lines of code coefficient) - moderate weight
GAP = 30.0           # Torque threshold for starting new batch (much lower threshold)
...
def torque_cluster(df: pd.DataFrame, α: float, β: float,
    ...
    print(f"Parameters: alpha={α}, beta={β}, gap={gap}, break_on_merge={break_on_merge}, break_on_author={break_on_author}")
    ...
    for idx, row in df.iterrows():
        ...
        # Debug output for first few decisions (ASCII-only)
        if idx < 5:
            print(
                f"Commit {idx}: dt_sec={Δt:.1f}, dloc={Δloc}, torque={torque:.1f}, "
...
def run_torque_clustering(input_file, output_file, summary_file=None):
    """
    Function interface for TORQUE clustering that can be imported
    
    Args:
        input_file (str): Path to input CSV file with commit data
        output_file (str): Path to output CSV file for clustered data
        summary_file (str, optional): Path to summary text file
    ...
    try:
        ...
        # Save clustering summary if path provided
        if summary_file:
        ...
        print(f"❌ TORQUE clustering failed: {e}")
        import traceback
        traceback.print_exc()
        return False
