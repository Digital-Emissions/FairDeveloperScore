...
class BatchImportanceCalculator:
    ...
    def normalize_importance_metrics(self, batch_metrics: pd.DataFrame) -> pd.DataFrame:
        ...
        # Handle single-batch scenario
        if len(batch_metrics) == 1:
            logger.info("Single batch detected - using default importance normalization")
            for col in raw_columns:
                if col in batch_metrics.columns:
                    z_col = col.replace('_raw', '_z')
                    # For single batch, use normalized raw value (between 0 and 1)
                    raw_value = batch_metrics[col].iloc[0]
                    if raw_value > 0:
                        batch_metrics[z_col] = 1.0  # Give meaningful importance to single batch
                    else:
                        batch_metrics[z_col] = 0.5  # Default moderate importance
        else:
            # Normal multi-batch z-score normalization
            for col in raw_columns:
                if col in batch_metrics.columns:
                    z_col = col.replace('_raw', '_z')
